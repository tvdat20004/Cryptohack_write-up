...
base = generate_basis(64)
def bypass_miller_rabin(base):
    def xgcd(a, b):
        s = 0
        t = 1
        r = b
    ...
    for tup in itertools.product(*fool2):
        ...
        for i, t in enumerate(tup):
            residues.append(t)
            modulos.append(base[i]*4)
            # g = GCD(base[i]*4, modul)
            # modul *= (base[i]*4)//g
...
import json 
from pwn import * 
fin,facs = bypass_miller_rabin(base)
send = json.dumps({
    "prime" : fin,
    "base" : facs[0]
})
...
r.sendlineafter(b'Oh Mr. Darcy, send me your primes!\n',send.encode())
r.interactive()